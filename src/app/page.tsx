...
import { Heart, Utensils, Sparkles, Smartphone, Users, MapPin, Clock } from "lucide-react";
        ...
        <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4 py-8 text-center">
          ...
          <div className="flex flex-col w-full max-w-sm sm:max-w-md md:max-w-lg gap-3 sm:gap-4 animate-fade-in px-4" style={{ animationDelay: '1s' }}>
            <Link href="/auth/sign-in" passHref>
              <Button size="lg" className="w-full bg-white text-purple-600 hover:bg-gray-100 font-semibold py-3 sm:py-4 text-base sm:text-lg rounded-full shadow-2xl transform hover:scale-105 transition-all duration-300">
                Start Swiping
              </Button>
            </Link>
            <Link href="/auth/sign-up" passHref>
              <Button size="lg" variant="outline" className="w-full border-2 border-white text-white hover:bg-white hover:text-purple-600 font-semibold py-3 sm:py-4 text-base sm:text-lg rounded-full shadow-2xl transform hover:scale-105 transition-all duration-300">
                Create Account
              </Button>
            </Link>
          </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
            {features.map((feature, index) => (
              <Card key={index} className="group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 sm:hover:-translate-y-2 border-0 shadow-lg">
